...
- Telemetry is sent by the device on its own and is **not** acknowledged. For
  ...
  after `0xFF`.
- OTA uses explicit `OTA_STATUS` messages (§9).
...
3. **Transfer.** The app sends `OTA_CHUNK` messages, each `[offset][data]`. The
   device replies `OTA_STATUS / ACK` with the next offset it expects. Firmware
   may acknowledge every chunk or use a fixed acknowledgement window, but every
   ACK must report the next expected offset. Chunk data must not exceed the
   accepted chunk size or the per-message payload limit.
   ...
   processing. The app should request a larger MTU before starting OTA to reduce
   write count.
...
waiting for `Transferring` — it only blocks on `ACK` (`0x03`) before advancing
to the next chunk or window.
...
| Transferring | first chunk received                   | accepts chunks, sends per-chunk or windowed acknowledgements      | `OTA_COMMIT`                                       |
...
and the STM32, not only by BLE itself. The accepted chunk size and
acknowledgement window should be tuned during bring-up against ATT MTU 23,
larger negotiated MTUs, UART speed/flow control, flash-write constraints, and
buffer size. The values used at first are starting points, not final figures.
