...
$PreviewDir = [IO.Path]::GetFullPath($PreviewDir)
...
function Add-StatusFormatting($range) {
    ...
    foreach ($item in $items) {
        $condition = Register-ExcelCom ($range.FormatConditions.Add(1, 3, ('="' + $item[0] + '"')))
...
function Add-TestSheet($workbook, [string]$name, [string]$title, [string]$subtitle, $rows, [string]$tableName) {
    $sheet = Register-ExcelCom ($workbook.Worksheets.Add())
    $sheet.Name = $name
    ...
    $headerRange = Register-ExcelCom ($sheet.Range('A4:M4'))
    ...
    $dataRange = Register-ExcelCom ($sheet.Range("A4:M$endRow"))
    ...
    $table = Register-ExcelCom ($sheet.ListObjects.Add(1, $dataRange, $null, 1))
    $table.Name = $tableName
    $table.TableStyle = 'TableStyleMedium2'
    $table.ShowAutoFilter = $true

    $statusRange = Register-ExcelCom ($sheet.Range("J5:J$endRow"))
    $retestRange = Register-ExcelCom ($sheet.Range("M5:M$endRow"))
...
try {
    ...
    $workbook = Register-ExcelCom ($excel.Workbooks.Add())

    while ($workbook.Worksheets.Count -gt 1) {
        $workbook.Worksheets.Item($workbook.Worksheets.Count).Delete()
    }

    $validation = Register-ExcelCom ($workbook.Worksheets.Item(1))
    ...
    $overview = Register-ExcelCom ($workbook.Worksheets.Add())
    $overview.Name = '测试概览'
    ...
    $checkBook = Register-ExcelCom ($excel.Workbooks.Open($OutputPath, 0, $true))
    foreach ($sheetName in @('测试概览','UI操作逻辑','APP基础测试')) {
        $sheet = Register-ExcelCom ($checkBook.Worksheets.Item($sheetName))
