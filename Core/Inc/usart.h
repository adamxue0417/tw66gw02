...
#define COM1_RX_CAPACITY  256u
#define COM2_RX_CAPACITY   32u
...
typedef struct
{
	uint8_t *dmaBuf;             /* Buffer currently owned by RX DMA */
	uint8_t *rxBuf;              /* Stable snapshot consumed by a task */
	uint16_t capacity;
	volatile uint16_t rxLen;
	volatile uint16_t pollPos;
