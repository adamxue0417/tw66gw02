...
void channel_env_init(void)
{
    ...
    for (i = 0; i < USR_ADC_CH_NR; ++i)
    {
        ...
        ch_env[i].vol = 0u;
        ch_env[i].sum = 0u;
        /* DMA owns adc_value while continuous conversion is running. */
    }
}
...
/**
  ...
  * @note     None
  */
static uint8_t AcquireAdcBatch(void)
{
    uint16_t i, j;
    uint32_t started;
    channel_env_init();
...
int16_t Temp_Get(uint8_t channel)
{
	...
	if (channel >= USR_ADC_CH_NR) { return TempErr; }
...
/**
  ...
  * @note     None
  */
void Add_Temp(uint16_t arr[], uint8_t length, uint16_t sample)
{
...
/**
  ...
  * @note     None
  */
uint16_t Temp_Handle(uint16_t arr[], uint8_t length, uint16_t sample)
{
...
static void raw_temp_sort_asc(int16_t values[], uint8_t length)
{
    uint8_t i;
    for (i = 1u; i < length; i++) {
...
void TempGetTask(void)
{
     ...
     * CH1 -> battery level
     */
    probe_f_raw = (AcquireAdcBatch() != 0u) ? Temp_Get(0u) : TempErr;
    if ((probe_f_raw != TempErr) && (probe_f_raw != TempHigh) && (probe_f_raw != TempLow) && (probe_f_raw >= 0))
    {
        probe_f = (int16_t)ProbeAverage((uint16_t)probe_f_raw);
    }
    else
    {
        probe_f = probe_f_raw;
