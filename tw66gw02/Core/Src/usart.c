...
/* Communication structures for each UART */
TypeDefCOM COM1, COM2;  /* COM1: BT, COM2: Temp sensor 1 */
...
void _usart_callback(UART_HandleTypeDef *uart, DMA_HandleTypeDef *dma, TypeDefCOM *com)
{
    uint32_t remain;
...
uint16_t COM_TakeRx(TypeDefCOM *com, uint8_t *dest, uint16_t capacity)
{
  ...
  {
    length = com->rxLen;
    if (length > COM_RXSIZE) { length = COM_RXSIZE; }
...
{
  if ((data != 0) && (num != 0u)) { (void)HAL_UART_Transmit_DMA(&huart1, data, num); }
}

void USART2_SendData(uint8_t *data, uint16_t num)
{
  (void)USART2_SendData_DMA(data, num);
}
/* USER CODE END 1 */
